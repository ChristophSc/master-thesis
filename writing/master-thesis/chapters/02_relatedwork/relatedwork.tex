...
\begin{enumerate}
    ...
    The basic idea of TransE is that a functional relation $r$ corresponds to a translation of the embeddings, i.e. $h + r \thickapprox t$ when (h,r,t) holds.
    If it does not hold, $t$ should be far away from $h + r$
    leda to a energy-based framework $d(h+r, t)$ for some dissimilarity measure $d$ ($L_1$ or $L_2$ norm) \cite{TransE}.
    Therefore, to learn the embeddings a margin-based ranking criterion has to be minimized:
    $$\mathcal{L} = \sum_{(h,r,t) \in S)} \sum_{(h',r,t') \in S'_{(h,r,t))})} [\gamma + d(h + r, t) - d(h' + r, t')]_+$$
    for training set S of positive triples and corresponding negative triples from S' with 
    $$S'_{(h,r,t)} = \{(h',r,t)| h' \in \entities\} \cup \{(h,r,t')| t' \in \entities\}$$.
    
    scoring function
    Therefore, since positive triples have a lower distance and negative triples a higher one, the distance needs to be inverted such that the score reflects the likelihood of the triple to be true.
    This can be done either by defining 
    $score(h,r,t) = \gamma - distance(h,r,t)$ or 
    $score(h,r,t) = (-1) * distance(h,r,t)$
    ...
    Since \textsc{DistMult} and  \textsc{ComplEx} play an important role in our approach, we will describe them in more detail.
    



DistMult \cite{DistMult}
Tensor Factorization-Based Models whre the representations
are learned using neural networks with energy-based objectives.
Training objective is to minimize the margin-based ranking loss
$$\mathcal{L} = \sum_{(h,r,t) \in S)} \sum_{(h',r,t') \in S'_{(h,r,t))})}  max\{ S_{(h',r,t')} - S_{(h,r,t)} + 1\}$$
Scoring function: 
$$score(h,r,t) = \langle e_h, e_r, e_t \rangle$$ where $e_h$, $e_r$ and $e_t$ are embeddings of heads, relations and tails.\\
...
\cite{cai2017kbgan}
A \textit{score function} $f(h,r,t)$ assigns a score to every possible triple in the knowledge graph.
The estimated likelihood of a triple to be true depends only on its score given by the score function.
Since different \ac{KGE} models fomulate different designs for this scoring function, the interpretation of this score is also different which in turn leads to different training objectives
The two most common forms are \textbf{Marginal Loss function} and \textbf{Log-softmax function}:
\begin{enumerate}
    \item \textbf{Marginal loss function}
    
    
    \item \textbf{Log-softmax loss function}
