...
%
The learning curves of MRR and Hit@10 values at the validation set on the \umls dataset can be seen in \Autoref{fig:advtrain_umls_usmax_ussoftmax}.
\begin{figure}[H]
    \centering
...
However, MRR values have a higher maximum with the \ussoftmax method.
...
\end{figure}
If the discriminator is certain about distinguishing these sampled negative triples from positives, later on, some others would be helpful.
Since the discriminator returns a negative distance of negative triple as a reward to the generator and the distance was small, the reward might be high.
Therefore, the generator continues to sample always the same triples without ever considering sampling other triples.
...
Even if training losses of discriminator increase after 150 epochs, the generator continues to sample the same triples since rewards increase over time and stay on the same level after about 300 epochs.
