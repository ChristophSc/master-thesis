...
Having established the binary classification problem between positive and negative triples, it is now necessary to determine how to calculate for a triple the probability of belonging to one of these classes.
...
Since $score_{min}$ and $score_{max}$ are now defined by option two, the probability of a triple to be positive ($y = 1$) can be calculated with the following \autoref{eqn:positive_probability}
\begin{equation}  \label{eqn:positive_probability}
    \mathds{P}(y = 1|(h, r, t)) =
    \begin{cases}
        0 \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \  
        if \  f_G(h,r,t) \leq score_{min}
         
        \\ \\
        1 \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ 
        if \ f_G(h,r,t) \geq score_{max}
