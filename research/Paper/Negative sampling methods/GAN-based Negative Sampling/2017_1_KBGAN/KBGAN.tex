...
%The objective of the generator is to generate difficult negative samples that can maximize their likeliness determined by the discriminator, while the discriminator minimizes its training loss.
...
%WW: nope, it should be low-dimensional, dense vectors.
%high dimensional 
...
%WW: In related work, you don't talk about your experiments.
%In the experiment, we improve \textsc{TransE} and \textsc{TransD} with adversarial training, using \textsc{DistMult} and \textsc{ComplEx} as the generator.
...
%WW: I don't understand this sentence. IRGAN does not deal with knowledge graph, right?
%and our framework also combines two types of knowledge graph embedding models like \textsc{IRGAN}.
...
%Models in this category formulate the scoring function as distance in the embedding space. When comparing multiple triples, the one with smaller distance is considered more likely to be true. However, the distance alone does not have a specific meaning.
...
%WW: I don't under the sentences below.
%Even if we know nothing about American geography, we know that ``?'' must be a geographical region --- the \emph{type} of the tail entity is constrained. Now, if our knowledge graph is not specialized in geography, and
%Some knowledge graphs have types (e.g., people, organization, location) associated with entities. 
...
\small
 ...
 \Repeat{convergence}{
  ...
  \For{$(h,r,t)\in \mathcal{T}_{batch}$}{
    ...
    Obtain their probability of being generated: $p_i=\frac{\exp f_G(h_i', r, t_i')}{\sum_{j=1}^{N_s} \exp f_G(h_j',r,t_j')}$\;
    Sample one negative triple $(h_s',r,t_s')$ from $Neg(h,r,t)$ according to $\{p_i\}_{i=1\dots N_s}$. Assume its probability to be $p_s$\;
...
%$R_G$ involves a discrete sampling step, so it cannot be directly optimized by gradient-based algorithms. However, we notice that the specific form of this objective, \emph{maximizing the expectation of a given function of samples from a parametrized probability distribution}, is identical to the objective of a one-step reinforcement learning problem. Using the terminology of reinforcement learning to paraphrase the problem, $(h,r,t)$ is the \emph{state}, $p_G(h',r,t'|h,r,t)$ is the \emph{policy}, $(h',r,t')$ is the \emph{action}, and $-f_D(h',r,t')$ is the \emph{reward}.
...
%Because pre-training a models is the same as training a single-model link predictor, generators should already be able to give high scores to "hard" negatives even without adversarial training. It is natural to question whether the adversarial training stage can really improve the quality of generated negatives. To answer such question, we attempted training the discriminator with a static generator. To be precise, we run the \textsc{kbgan} algorithm in exactly the same way as previous experiments, except that we never update parameters of the generator.

%Results of such attempts are summarized in the lowermost section of Table \ref{tab:results}. We notice that even the "static" setting can consistently improve the discriminator, which proves that it is reasonable to raise such question. However, the normal setting also outperforms the "frozen" setting consistently. From these results, we can draw a conclusion that adversarial training does improve the quality of negatives and help training the discriminator.
